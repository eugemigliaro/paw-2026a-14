...
public class ActionVerificationServiceImpl implements ActionVerificationService {
    ...
    private VerificationPreview buildMatchCreationPreview(
        ...
        details.add(
                new VerificationPreviewDetail(
                        message("verification.preview.detail.schedule", locale),
                        formatSchedule(Instant.ofEpochMilli(payload.getStartsAtEpochMillis()), locale)));
        if (payload.getEndsAtEpochMillis() != null) {
            details.add(
                    new VerificationPreviewDetail(
                            message("verification.preview.detail.endTime", locale, "End time"),
                            formatEndTime(
                                    Instant.ofEpochMilli(payload.getEndsAtEpochMillis()),
                                    locale)));
        }
    ...
    private String formatEndTime(final Instant endsAt, final Locale locale) {
        return DateTimeFormatter.ofLocalizedTime(FormatStyle.SHORT)
                .withLocale(resolvedLocale(locale))
                .format(endsAt.atZone(ZoneId.systemDefault()));
    }
    ...
    private String message(final String code, final Locale locale, final String defaultMessage) {
        return message(code, null, resolvedLocale(locale), defaultMessage);
    }
