...
public class MatchDashboardController {
    ...
    public ModelAndView showHostedMatches(
        ...
        if (email == null || email.isBlank()) {
            return redirectTo("/host/matches/new", locale);
        }
    ...
    public ModelAndView showPastJoinedMatches(
            ...
            final Locale locale) {
        if (email == null || email.isBlank()) {
            return redirectTo("/explore", locale);
        }
    ...
    private static ModelAndView redirectTo(final String path, final Locale locale) {
        final UriComponentsBuilder builder = UriComponentsBuilder.fromPath(path);
        if (locale != null && locale.getLanguage() != null && !locale.getLanguage().isBlank()) {
            builder.queryParam("lang", locale.getLanguage());
        }
        return new ModelAndView("redirect:" + builder.build().encode().toUriString());
    }
