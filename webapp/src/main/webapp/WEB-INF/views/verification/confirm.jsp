...
<body>
	...
	<main class="page-shell verification-shell">
		<section class="panel verification-panel">
			...
			<h1 class="page-heading__title"><spring:message code="verification.preview.account.title" /></h1>
			<p class="page-heading__description"><spring:message code="verification.preview.account.summary" /></p>
			<p class="muted-copy">
			...
			<spring:message var="confirmLabel" code="verification.preview.account.confirm" />
			<c:url var="confirmAction" value="${confirmPath}" />
			<form method="post" action="${confirmAction}">
				<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}" />
				<ui:button label="${confirmLabel}" type="submit" fullWidth="${true}" />
			</form>
		</section>
	</main>
	</div>
